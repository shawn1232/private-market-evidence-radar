...
`public_live` 是为作品集单独设置的窄化公网配置：只允许页面读取，以及同源触发、带全局冷却的 `POST /api/refresh`。该刷新仅运行程序内置的 Google News RSS 查询；不会调用公众号登录、MCPorter/Exa、第三方正文回退、任意 URL 抓取、文件导入、删除或深度评分任务。

云端返回的真实名称、日期与来源仍全部标记为 `discovery_only / 待核验`，不能冒充已读原文或投资结论。个人账号、Cookie、本地公众号池和研究资料不会进入部署镜像或公共刷新链路。
