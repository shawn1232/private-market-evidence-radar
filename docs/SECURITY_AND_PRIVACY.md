...
`public_live` 是为作品集单独设置的窄化公网配置：只允许页面读取，以及同源触发、带全局冷却的 `POST /api/refresh` 和 `POST /api/wechat/discover`。前者仅运行程序内置的 Google News RSS 查询；后者将三组预设行业/事件查询发送到公开 Exa MCP，并把最多24条公众号文章 URL 与搜索元数据写入独立云端文章池。它不会调用公众号登录、本地 MCPorter 配置、第三方正文回退、任意 URL 抓取、文件导入、删除或深度评分任务。

云端返回的真实名称、日期与来源仍全部标记为 `discovery_only / 待核验`，不能冒充已读原文或投资结论。Exa 搜索摘要不会写成文章正文或发布日期；个人账号、Cookie、本地公众号池和研究资料不会进入部署镜像或公共刷新链路。
